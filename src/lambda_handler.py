...
class FaultInjector:
    ...
    SCENARIOS = {
        ...
        "db_pool_exhaustion": {
            "db_timeout_rate": 0.50,  # 50% failures
...
def simulate_database_query(order_id: str, user_id: str) -> Dict[str, Any]:
    ...
    if fault_roll < fault_config.get("db_timeout_rate", 0):
        ...
        logger.error(
            "Connection timeout to database orders_db",
            extra={
                "order_id": order_id,
                "user_id": user_id,
                "database": "orders_db",
                ...
                "connection_pool_size": 10 if scenario == "db_pool_exhaustion" else 50
            }
        )
        
        metrics.add_metric(name="DatabaseErrors", unit=MetricUnit.Count, value=1)
        raise TimeoutError(f"Database connection timeout after {query_duration}ms")
    
    # Inject slow query
    elif fault_roll < fault_config.get("db_timeout_rate", 0) + fault_config.get("slow_query_rate", 0):
        slow_duration = random.randint(1500, 2500)
        time.sleep(slow_duration / 1000)
    ...
    elif fault_roll < (fault_config.get("db_timeout_rate", 0) + 
